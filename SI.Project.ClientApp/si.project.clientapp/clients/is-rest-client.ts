...
export module back_end {
    ...
    export class Client {
        ...
        /**
         * @param usernameQuery (optional) 
         * @return Success
         */
        apiUsersSearch(usernameQuery: string | undefined): Promise<GetUserDto[]> {
            let url_ = this.baseUrl + "/api/Users/search?";
            if (usernameQuery === null)
                throw new Error("The parameter 'usernameQuery' cannot be null.");
            else if (usernameQuery !== undefined)
                url_ += "usernameQuery=" + encodeURIComponent("" + usernameQuery) + "&";
            url_ = url_.replace(/[?&]$/, "");
            ...
            return this.http.fetch(url_, options_).then((_response: Response) => {
                return this.processApiUsersSearch(_response);
            });
        }
    
        protected processApiUsersSearch(response: Response): Promise<GetUserDto[]> {
            const status = response.status;
            let _headers: any = {}; if (response.headers && response.headers.forEach) { response.headers.forEach((v: any, k: any) => _headers[k] = v); };
            if (status === 200) {
                return response.text().then((_responseText) => {
